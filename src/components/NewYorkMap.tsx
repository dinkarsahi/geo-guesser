...
import { needsCasing } from "../data/tube";
...
const SNAP_PX = 20;
/**
 * How many rings of shading the map is drawn in.
 *
 * **None, on this map.** London's six are its fare zones — a real thing about
 * that network, nested, and readable as rings. New York has a flat fare and no
 * zones at all, and the honest translation of "no zones" turned out to be no
 * shading rather than one band: a single convex hull round every station is a
 * grey wedge across the harbour that says nothing, and drew the eye away from
 * the network it was supposed to sit behind. The boroughs underneath are what
 * gives this map its ground.
 *
...
/**
 * No landmarks on this one yet.
 *
 * London's two — Big Ben and Tower Bridge — are drawn icons, and New York's
 * equivalents would each be a drawing rather than a data change. Kept as an
 * empty list rather than torn out, so adding them later is a list and not a
 * refit.
 */
const LANDMARKS: { name: string; lat: number; lng: number; icon: string; base: number }[] = [];

/**
 * No river ribbon on this map, and that is a decision rather than a gap.
 *
 * London gets one because the Thames is the single most recognisable thing on
 * that map and the borough outlines behind it are faint to the point of
 * decoration. New York's shoreline is already drawn: the water here *is* the
 * gap between the five borough shapes, every inlet of it, from a boundary file
 * rather than from anybody's memory. A hand-drawn Hudson on top of that was
 * both redundant and — as it turned out on screen — visibly wrong, running
 * diagonally across Manhattan because it had been guessed at rather than
 * traced. Two lines of geography disagreeing is worse than one.
 */
...
const stripes = stripeConnections(subwayLines, subwayConnections);
...
interface NewYorkMapProps extends GuessMapProps {
...
export default function NewYorkMap({
  ...
  // The London map's own names, kept: this is that component with New York's
  // geography swapped in, and a copy that has been reworded is a copy nobody
  // can diff against the thing it was copied from.
  const tubeStations = subwayStations;
  const tubeConnections = subwayConnections;
  const tubeLines = subwayLines;
  const stationCoords = subwayCoords;
  const stripedConnections = stripes;
        ...
        >
          ...
          >
            ...
            <g transform={`translate(${anchor[0] * (1 - s)} ${anchor[1] * (1 - s)}) scale(${s})`}>
              <Geographies geography={BOROUGHS_URL}>
                {({ geographies }) =>
                  geographies.map((geo) => (
                    <Geography
                      key={geo.rsmKey}
